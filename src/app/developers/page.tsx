  ...
  return (
    <div className=" mb-8 flex flex-wrap justify-start gap-10">
      {data.map((value, index) => (
        <div
          key={index}
          className="relative shadow-lg w-[400px] rounded-xl overflow-clip  h-[500px] flex-col justify-between mx-auto "
        >
          ...
          <div className=" flex justify-between p-4 items-center bg-white absolute bottom-0 z-10 w-full">
            ...
            <div className="flex gap-4">
              <Link href={value.linkedin} target="_blank">
                <LinkedInIcon />
              </Link>
              <Link href={value.github} target="_blank">
                <GitHubIcon />
              </Link>
              <Link href={`mailto:${value.email}`} target="_blank">
                <EmailIcon />
              </Link>
              <Link href={value.website ? value.website : "#"} target="_blank">
                <LanguageIcon />
              </Link>
            </div>
          </div>
